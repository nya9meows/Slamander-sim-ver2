...
for i in range(numofmodules):
    ...
    # boxId = p.loadURDF("./segf_ver4/urdf/segf_ver4.urdf", startPos, startOrientation)

    listboxId.append(boxId)
    if i > 0:
        cid = p.createConstraint(listboxId[i-1], -1, listboxId[i], 5, p.JOINT_FIXED, [0.0, 0.0, 0.0], [0.05, 0, 0], [-0.05, 0, 0],
                                 [0, 0, 0],
                                 [0, 0, 0])
        listcid.append(cid)
        pivot = [0, 0, 0]
        ...
        p.changeConstraint(listcid[i-1], pivot, jointChildFrameOrientation=orn, maxForce=50)
